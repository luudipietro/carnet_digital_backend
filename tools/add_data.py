    ...
    def estandarizar_columnas(df, tipo):
        ...
        if tipo == 'socios':
            ...
            for c in cols:
                ...
                # if 'nombre' == c:
                if 'nombre_apellido' not in encontrados and any(x in c for x in ['cliente' ,'nombre' ,'socio' ,'apellido']):
                    nuevos_nombres[cols[c]] = 'nombre_apellido'
                    encontrados.add('nombre_apellido')
            ...
            for c in cols:
                if 'nombre_apellido' not in encontrados and c =='cliente':
                    nuevos_nombres[cols[c]] = 'nombre_apellido'
                    encontrados.add('nombre_apellido')
                if 'monto_deuda' not in encontrados and c == 'importe mon. ppal.':
